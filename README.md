...
This repository ships seventy-nine small, tool-free Agent Skills for evidence-first PM
work:
...
| An AI policy or regulation may affect a product | [`pm-ai-policy-to-product`](./skills/pm-ai-policy-to-product/SKILL.md) | Applicability frame, requirement ledger, product control map, owner/evidence receipt, exceptions, freshness, and Map controls/Validate applicability/Hold/Do not claim routes |
| An AI capability needs a maintained system record | [`pm-ai-system-to-inventory`](./skills/pm-ai-system-to-inventory/SKILL.md) | Stable identity, purpose/scope, actors/owners, users/affected parties, surfaces/dependencies, data/model/tool/permission links, lifecycle, evidence, review, change, and retirement routes |
| An external AI provider dependency needs a bounded decision | [`pm-ai-vendor-to-decision`](./skills/pm-ai-vendor-to-decision/SKILL.md) | Provider/entity/service role, contract/source scope, data/region/tenant, model/tool lifecycle, security/privacy, support/limits, cost, lock-in/portability, owners, receipts, and Compare bounded/Need due diligence/Hold/Exit or replace routes |
| An AI workflow needs human-review coverage and capacity | [`pm-ai-review-to-capacity`](./skills/pm-ai-review-to-capacity/SKILL.md) | Review unit, consequence, full/sample/exception coverage, queue demand, reviewer capacity, quality/calibration boundary, privacy/access, escalation, fallback, economics, owner, and Cover required/Sample and monitor/Add capacity/Hold routes |
...
Use the smallest matching skill first; do not chain all seventy-nine before
...
- [`pm-ai-adoption-to-diagnosis`](./skills/pm-ai-adoption-to-diagnosis/SKILL.md)
  diagnoses why one tested workflow is not becoming repeatable real work. It
  separates exposure, first use, repeat use, changed work, and outcome; maps
  fit, trust/quality, access, ownership/reinforcement, process/environment,
  repeatability/packaging, value evidence, and measurement gaps; then proposes
  one smallest `Try`, `Instrument`, `Escalate`, `Hold`, or `Stop expanding`
...
- [`pm-ai-policy-to-product`](./skills/pm-ai-policy-to-product/SKILL.md) turns an
  external AI policy, standard, regulation, contract, or governance requirement
  into a source-bounded applicability frame and product control map. It keeps
  actor role, jurisdiction, system type, surface, effective date, modal force,
  owner, evidence receipt, exception, freshness, and fallback visible without
  giving legal advice or calling a product compliant. Start with its [fictional
  AI support policy map](./skills/pm-ai-policy-to-product/examples/first-run.md)
  or read the [policy-to-product control map](./skills/pm-ai-policy-to-product/references/policy-control-map.md).
- [`pm-ai-system-to-inventory`](./skills/pm-ai-system-to-inventory/SKILL.md)
  turns one AI capability, agent, model-backed feature, or vendor integration
  into a source-bounded lifecycle inventory record. It keeps stable identity,
  purpose and scope, actors and owners, users and affected parties,
  surfaces/dependencies, data/model/tool/permission links, lifecycle state,
  evidence, freshness, changes, incidents, review, and retirement visible
  without treating a complete form as proof of safety, compliance, adoption,
  value, or production readiness. Start with its [fictional support-assistant
  inventory](./skills/pm-ai-system-to-inventory/examples/first-run.md) or read
  the [system inventory reference](./skills/pm-ai-system-to-inventory/references/system-inventory.md).
- [`pm-ai-vendor-to-decision`](./skills/pm-ai-vendor-to-decision/SKILL.md)
  turns one external AI provider or vendor dependency into a source-bounded
  decision packet for a named job and scope. It keeps provider/entity/service
  roles, contract and source authority, data/use/region/tenant, model/tool
  lifecycle, security/privacy, support/limits, economics, lock-in/portability,
  owners, implementation receipts, and exit conditions visible without ranking
  vendors or claiming a recommendation, legal conclusion, or readiness. Start
  with its [fictional provider dependency decision](./skills/pm-ai-vendor-to-decision/examples/first-run.md)
  or read the [vendor decision reference](./skills/pm-ai-vendor-to-decision/references/vendor-decision.md).
- [`pm-ai-review-to-capacity`](./skills/pm-ai-review-to-capacity/SKILL.md)
  turns “human-in-the-loop” into a source-bounded review-operations packet for
  one AI output or agent job. It keeps the review unit, consequence, coverage
  and sampling policy, queue demand, reviewer capacity, quality/calibration
  boundary, privacy/access, escalation, fallback, economics, owners, and
  receipts visible without treating a review plan as quality, safety, adoption,
  or launch proof. Start with its [fictional AI review queue](./skills/pm-ai-review-to-capacity/examples/first-run.md)
  or read the [review capacity reference](./skills/pm-ai-review-to-capacity/references/review-capacity.md).
...
**Public skill pilot:** Try one of the seventy-nine first runs with a real, sanitized note, then
