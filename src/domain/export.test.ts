...
describe("decision memo export", () => {
  ...
  it("includes evidence, uncertainty, experiment and not-covered sections", () => {
    ...
    if (result.ok) {
      ...
      expect(markdown).toContain("You decide whether to use it");
      expect(markdown).not.toContain("model quality proof");
      expect(markdown).not.toContain("real model quality");
    }
  });
});
